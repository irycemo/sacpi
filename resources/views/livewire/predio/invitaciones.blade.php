<div class=" text-center">
    ...
    <div class="bg-white rounded-lg shadow-xl p-4 w-full lg:w-1/2 mx-auto mb-5">
        ...
        <x-button wire:loading.attr="disabled" wire:click="generarInvitacion">
            Generar invitación
        </x-button>
    ...
    <div class="bg-white rounded-lg shadow-xl p-4 w-full lg:w-1/2 mx-auto">
        ...
        <x-table>
            ...
            <x-slot name="body">
                ...
                @forelse ($predio->invitaciones as $invitacion)

                    <x-table.row wire:loading.class.delaylongest="opacity-50" wire:key="row-{{ $invitacion->id }}">
                        <x-table.cell>
                            <span class="lg:hidden absolute top-0 left-0 bg-blue-300 px-2 py-1 text-xs text-white font-bold uppercase rounded-br-xl">Folio</span>
                            <span class="whitespace-nowrap">{{ $invitacion->año }}-{{ $invitacion->folio }}-{{ $invitacion->usuario }}</span>
                        </x-table.cell>
                        <x-table.cell>
                            ...
                            <span class="whitespace-nowrap">{{ $invitacion->fecha_notificacion_formateada ?? 'N/A' }}</span>
                        </x-table.cell>
                        <x-table.cell>
                            ...
                            <span class="whitespace-nowrap">${{ number_format($invitacion->monto, 4) }}</span>
                        </x-table.cell>
                        <x-table.cell>
                            ...
                            <span class="whitespace-nowrap">{{ $invitacion->created_at }}</span>
                        </x-table.cell>
                        <x-table.cell>
                            ...
                            <div class="ml-3 relative" x-data="{ open_drop_down:false }">
                                ...
                                <div x-cloak x-show="open_drop_down" x-on:click="open_drop_down=false" x-on:click.away="open_drop_down=false" class="z-50 origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu">
                                    ...
                                    @if(!$invitacion->fecha_notificacion)

                                        <button
                                            wire:click="notificar({{ $invitacion->id }})"
                                            wire:confirm="¿Esta seguro que desea notificar la invitación?"
